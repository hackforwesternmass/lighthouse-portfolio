  ...
  render(){
    ...
    return (
      <div id='goals'>
        ...
        { newGoal && <Goals.GoalForm {...this.props} parent={this} goal={{action_items: []}} newGoal={newGoal} /> }
        {
          goals.map(goal => {
...
Goals.GoalShow = React.createClass({
  ...
  toggleComplete(e){
    ...
    $.ajax({
      ...
      dataType: 'JSON',
      type: 'PATCH',
      data: { goal: { is_completed: !goal.is_completed } },
      success: () => {
        parent.loadGoals();
      },
      error: () => {
  ...
  progress() {
    ...
    action_items.forEach(actionItem => {
      if(actionItem.completed) counter += 1;
    });
      ...
      <div className='card'>
        <div className='card-content'>
          ...
          <div className='row'>
            ...
            <div className='col s6 right right-align'>{due_date ? moment(due_date).format('MMMM D YYYY') : '∞'}</div>
          </div>
            ...
            <div>
              ...
              <div className='row'>
                <div className='progress-bar col s12'>
                  <div className='completed-progress-bar' style={{ width: this.progress() + '%' }} ></div>
        ...
        <a href='#' className={is_completed ? 'complete-btn' : 'incomplete-btn'} onClick={this.toggleComplete}>{this.props.goal.is_completed ? 'Completed' : 'Finished?' }</a>
      </div>
    );
  }
});
...
Goals.GoalForm = React.createClass({
  getInitialState() {
    ...
    return { showChecklist: goal.action_items.length > 0, goal };
  },
  componentDidMount() {
    if(this.props.newGoal) {
      this.setState({ goal: update(this.state.goal, { action_items: { $push: [this.defaultActionItem()] } }) });
    }
    ...
    return(
      <div className='card'>
        <div className='card-date grey darken-3 white-text'>
          {created_at ? moment(created_at).format('MMMM D YYYY') : 'New Goal'}
          <a className='right white-text' onClick={this.closeGoal}><i className='fa fa-times'></i></a>
        </div>
        <div className='card-content'>
          <form onSubmit={this.handleSubmit} >
            ...
            <div className='row'>
              ...
              <div className='input-field col s12 m4'>
                <input type='text' className='datepicker' name='goal[due_date]' id='goal_due_date' defaultValue={due_date ? moment(due_date).format('D MMMM, YYYY') : ''} />
                <label htmlFor='goal_due_date'>Due Date</label>
              </div>
            </div>
            <div className='row no-margin'>
              <div className='col s12'>
                {!showChecklist && <a onClick={this.toggleChecklist} className='btn waves-effect waves-light grey darken-1'>{action_items.length > 0 ? 'Open Checklist' : 'Add Checklist'}</a>}
                {
                  showChecklist &&
                  <div>
                    <div className='row'>
...
Goals.ActionItem = React.createClass({
  ...
  handleRemove(e) {
    ...
    if(newActionItem) {
      parent.setState({ goal: update(parent.state.goal, { action_items: { $splice: [[index, 1]] } }) });
    } else {
  ...
  render() {
    ...
    return(
      <div className='row new-goal' style={removed ? { display: 'none' } : {}}>
