          ...
          <div id='category-grid' className='row'>
            {
              Object.keys(generalResources).map((category, i)=> {
                return <ResourceCategories
                          {...this.props}
                          updateResourceGroup={this.updateResourceGroup}
                          editing={editing}
...
var ResourceCategories = React.createClass({
  ...
  render(){
    ...
    const resourceNodes = this.state.resources.map((resource, i) => {
      ...
      if(this.state.general){
        if(this.state.editing && this.props.isAdmin){
          deleting = <a className='right' rel='nofollow' onClick={this.deleteResource} href={`/users/${this.props.userId}/resources/${resource.id}`}><i className='fa fa-trash-o'></i></a>;
          editing = <a className='right' href={`/users/${this.props.userId}/resources/${resource.id}/edit`}><i className='fa fa-pencil'></i></a>;
          }
        }else{
          if(this.state.editing){
            deleting = <a className='right' rel='nofollow' onClick={this.deleteResource} href={`/users/${this.props.userId}/resources/${resource.id}`}><i className='fa fa-trash-o'></i></a>;
