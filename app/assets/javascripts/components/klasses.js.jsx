...
Klasses.Search = React.createClass({
  ...
  Klasses.Show = React.createClass({
    ...
    studentNames() {
      ...
      if(klass.archive) {
        return klass.enrolls.filter(enrolly => !enrolly.user.archive).map(enrolly => enrolly.user.full_name);
      } else {
        return klass.enrolls.filter(enrolly => !(enrolly.completed || enrolly.user.archive)).map(enrolly => enrolly.user.full_name);
      }
    },
    render() {
      const { klass } = this.props;
      return(
        <tr className={klass.archive ? 'half-opacity' : ''}>
          ...
          <td className='hide-on-small-only'>
            <a href={`/class/${klass.id}.csv`} download={klass.name} className={this.studentNames().length > 0 ? 'tooltipped center-align' : 'center-align'} data-position='left' data-delay='50' data-tooltip={this.studentNames().join(', ')}>{this.studentNames().length}</a>
